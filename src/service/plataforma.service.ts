...
export class PlataformaService {
    ...
    public splitFilesForUploadInfo(files: Array<FileFinalPtBrInterface>): Array<FileGroupByCommitAndCategory> {
        const groupByCommit: Array<any> = [];
        files.forEach((file) => {
            const groupIndex = groupByCommit.findIndex((group) => group.hash_commit === file.hash_commit);
            
            if (groupIndex >= 0) {
                const categoryIndex = (groupByCommit[groupIndex].categorias as Array<any>).findIndex((category) => category.categoria === file.categoria.code);
                if (categoryIndex >= 0) {
                    groupByCommit[groupIndex].categorias[categoryIndex].arquivos.push(file.arquivo_com_hash);
                } else {
    ...
    public listFilesByProjectByCategories(files: Array<FileGroupByCommitAndCategory>) {
        ...
        files.forEach((group) => {
            group.categorias.forEach((category) => {
                category.arquivos.forEach((file) => {
                    ...
                    const projectIndex = filesByProject.findIndex((project) => project.nome_projeto === projectName);

                    if (projectIndex >= 0) {
                        const categoryIndex = (filesByProject[projectIndex].categorias as Array<any>).findIndex((c) => c.categoria === category.categoria);
                        if (categoryIndex >= 0) return filesByProject[projectIndex].categorias[categoryIndex].arquivos.push(file);
    ...
    private getCategoryByTypeAndNewFile(extension: string, isNewFile: boolean): FileCategoryInterface {
        switch (extension) {
            case 'js':
                return isNewFile 
            ...
            case 'java':
                return isNewFile 
                    ? { code: '5.10.9', description: 'Criação de arquivos Java' }
                    : { code: '5.10.10', description: 'Alteração de arquivos Java' }
            case 'css':
                return isNewFile 
            ...
            case 'sh':
                return isNewFile 
                    ...
                    : { code: '5.15.2', description: 'Alteração scripts (.sh ou outros)' }
            case 'md':
                return { code: '5.26.2', description: 'Construção/Alteração README e CHANGELOG e documentos auxiliares da aplicação' }
            default:
                return { code: '0.0.0', description: 'Arquivo não Mapeado ainda.'}
        }
    }
}